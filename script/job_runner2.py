...
def load_data(conf, sample=False):
    return data.Data.prefetch_data(conf, sample=sample)
...
def run_narx(job, weather_data, is_report_email=False):
...
def run_time(job, weather_data, is_report_email=False):

    def training(start, finish, horizons, predictor_fun_name, job):
        max_h = max(horizons)
        ...
        data_obj = data.WindData(start, finish,
                                 str(test_start),
                                 str(test_finish),
                                 skformat=True,
                                 data=weather_data)
        ...
        y_, cov = predictor_fun(data_obj, job=job)
        ...
        horizon_idx = np.array(horizons) - 1
        y_true = y_test[horizon_idx, :]
        y_pred = y_[horizon_idx, :]
        cov_ = cov[horizon_idx, :]
        mse_ = mse[horizon_idx, :]

        result = np.hstack([y_true, y_pred, mse_, cov_])
        return result
    ...
    for f, fold in enumerate(fold_detail):
        print fold
        final_result[i:i+len(horizons), :] = training(fold[0],
                                                      fold[1],
                                                      horizons,
                                                      predictor_fun_name,
                                                      job)
...
if __name__ == '__main__':
    ...
    weather_data = load_data(conf, sample=True)
    ...
    for job in job_spec['jobs']:
        ...
        job_type = job['type']
        if job_type == 'narx':
            run_narx(job, weather_data, False)
        elif job_type == 'time':
            run_time(job, weather_data, False)
