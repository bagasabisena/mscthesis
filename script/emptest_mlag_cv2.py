...
def predictor(x_train, y_train, x_test, kernel_setup):
    X = x_train
    y = y_train
    X_ = x_test

    kernels = []
    for setup in kernel_setup:
        kernels.append(GPy.kern.RBF(input_dim=setup[0],
                                    active_dims=setup[1],
                                    ARD=False))
...
def training(h, start, finish, output_feature, input_feature, weather_data, l=1, time_as_feature=False):
    ...
    kernel_setup = data_class.multikernel_setup()
    y_, cov, kernel = predictor(data_class.x_train,
                             data_class.y_train,
                             data_class.x_test,
                             kernel_setup)
    ...
    lengthscales = np.array([param.param_array[1] for param in kernel.parameters])
